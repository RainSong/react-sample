...
//gulp主动设置的命令
gulp.task("left_nav", function () {
    // 通过 Browserify管理依赖
    browserify({
        // 入口点,app.jsx
        entries: ["public/static/scripts/src/leftnav.jsx"],
    ...
    //转换为gulp能识别的流
        ...
        //输出Map文件
        .pipe(exorcist(path.join(__dirname, "/public/static/scripts/build/leftnav.js.map")))
        //输出的目录
        //.pipe(gulp.dest("public/static/scripts/build/pages/"))
        .pipe(fs.createWriteStream(path.join(__dirname, "/public/static/scripts/build/leftnav.js")), "utf-8");
});
...
gulp.task("admin_datasource_list", function () {
    // 通过 Browserify管理依赖
    browserify({
        // 入口点,app.jsx
        entries: ["public/static/scripts/src/pages/admin.datasource.list.jsx"],
        // 利用Reactify工具将jsx转换为js
        transform: [reactify],
        debug: true
    })
    //转换为gulp能识别的流
        ...
        //.pipe(source("admin.modules.js"))
        //输出Map文件
        .pipe(exorcist(path.join(__dirname, "/public/static/scripts/build/pages/admin.datasource.list.js.map")))
        //输出的目录
        //.pipe(gulp.dest("public/static/scripts/build/pages/"))
        .pipe(fs.createWriteStream(path.join(__dirname, "/public/static/scripts/build/pages/admin.datasource.list.js")), "utf-8");
});

//gulp默认命令
gulp.task("default", ["left_nav","admin_module_list","admin_module_addedit","admin_datasource_list"]);
